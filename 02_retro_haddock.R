...
eval_retro <- function(x, y, type = c("M_inc", "index_mult", "index_omit", "M_age", "catch_mult", "M_inc2", "sigmaR"), years_I, years = NULL, args) {
  require(dplyr)
...
#ret_cat_mult[[2]] <- sfClusterApplyLB(c_mult, eval_retro, years = 38:42, args = args) # 5
#ret_cat_mult[[3]] <- sfClusterApplyLB(c_mult, eval_retro, years = 28:42, args = args) # 15
#
...
rho <- cbind %>% do.call(lapply(sR, function(x) summary(x[[2]])[2, 1]))
...
# Compare haddock
res <- readRDS("GoM_haddock/haddock_sel9.rds")
res2 <- readRDS(file = "GoM_haddock/haddock_no_ret.rds")
res3 <- readRDS(file = "GoM_haddock/haddock_no_ret2.rds")
res4 <- readRDS(file = "GoM_haddock/haddock_no_ret3.rds")


compare_SRA(res, res2, res3, res4,
            filename = "compare_haddock_ret", dir = "GoM_haddock", title = "ASAP3 haddock comparison",
            s_name = c("NEFSCspring", "NEFSCfall"), open_file = FALSE,
            scenario = list(names = c("Base", "Imult", "M decrease", "CandImult")))



SSB <- cbind %>% do.call(lapply(list(res, res2, res3, res4), function(x) x@SSB[1, ]))

png("GoM_haddock/SSB.png", height = 4, width = 5, units = "in", res = 400)
par(mar = c(5, 4, 1, 1))
matplot(1977:2019, SSB, xlab = "Year", type = "l", lty = 1, lwd = 2, ylim = c(0, 1e5))
abline(h = 0, col = "grey")
legend("topleft", c("R1", "NR1", "NR2", "NR3"), pch = 16, col = 1:4)
dev.off()

RR <- cbind %>% do.call(lapply(list(res, res2, res3, res4), function(x) x@Misc[[1]]$R))

png("GoM_haddock/Recruits.png", height = 4, width = 5, units = "in", res = 400)
par(mar = c(5, 4, 1, 1))
matplot(1977:2019, RR, ylab = "Recruitment", xlab = "Year", type = "l", lty = 1, lwd = 2, ylim = c(0, 1.7e5))
abline(h = 0, col = "grey")
legend("topleft", c("R1", "NR1", "NR2", "NR3"), pch = 16, col = 1:4)
dev.off()

setup(4)
rr <- sfLapply(list(res, res2, res3, res4), retrospective, nyr = 7, figure = FALSE)

png("GoM_haddock/rho.png", height = 4, width = 6.5, units = "in", res = 400)
par(mfcol = c(2, 4), mar = c(4, 4, 1, 1))
mod_vec <- c("R1", "NR1", "NR2", "NR3")
for(i in 1:4) {
  matplot(rr[[i]]@TS[, , 1] %>% t(), type = 'l', col = gplots::rich.colors(7), lty = 1, ylim = c(0, 1.5), 
          xlab = "Year", ylab = "F")
  legend("topleft", c(mod_vec[i], latex2exp::TeX(paste0("$\\rho = ", summary(rr[[i]])[1,1], "$"))), bty = "n")
  matplot(rr[[i]]@TS[, , 2] %>% t(), type = 'l', col = gplots::rich.colors(7), lty = 1, ylim = c(0, 1e5), 
          xlab = "Year", ylab = "SSB")
