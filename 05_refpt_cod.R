...
ggplot(indicators_plot, aes(x = Year, shape = OM, colour = OM)) + 
  geom_hline(data = data.frame(Ind = factor("SSB_rho", levels = c("SSB_rho", "Cat_slp", "Cat_mu", "Ind_1_slp", "MAge_1_slp")), yy = 0), 
...
ggplot(filter(indicators_scatter, Year == 2027), aes(x = SSB_rho, y = Cat_slp, shape = OM, colour = OM)) + geom_point() + 
  facet_wrap(~ factor(MP, levels = c("M02", "M02_ra", "MRAMP", "MRAMP_ra", "ma")), scales = "free") + ggtitle("Year 2027") + 
  gfplot::theme_pbs() + legend_bottom
ggsave("report/GoM_cod/indicator_scatter.png", height = 4.5, width = 7)


ggplot(filter(indicators_scatter, Year == 2045), aes(x = SSB_rho, y = Cat_slp, shape = OM, colour = OM)) + geom_point() + 
  facet_wrap(~ factor(MP, levels = c("M02", "M02_ra", "MRAMP", "MRAMP_ra", "ma")), scales = "free") + gfplot::theme_pbs() + legend_bottom
...
s_CAA_hist <- get_cod_M02()$data$s_CAA
indicators_raw <- lapply(1:length(MSE), get_indicators, ind_interval = 6, MSE = MSE, 
                         MPs = paste0("FMSY", seq(0.5, 3, 0.25)), Cbias = c(2.25, 1.25, 1), s_CAA_hist = s_CAA_hist,
                         mah_ind = c("SSB_rho", "Cat_slp", "Cat_mu", "Ind_1_slp", "MAge_1_slp"))
indicators <- do.call(rbind, lapply(indicators_raw, getElement, 1))
...
indicators_plot <- filter(indicators_ts, match(Ind, c("SSB_rho", "Cat_slp", "Cat_mu", "Ind_1_slp", "MAge_1_slp"), nomatch = 0) %>% as.logical()) %>%
  mutate(MP = factor(MP, levels = c("M02", "M02_ra", "MRAMP", "MRAMP_ra", "ma"))) %>%
  mutate(Ind = factor(Ind, levels = c("SSB_rho", "Cat_slp", "Cat_mu", "Ind_1_slp", "MAge_1_slp"))) #%>% filter(Year >= 2024)
ggplot(indicators_plot, aes(x = Year, shape = OM, colour = OM)) + 
  geom_hline(data = data.frame(Ind = factor("SSB_rho", levels = c("SSB_rho", "Cat_slp", "Cat_mu", "Ind_1_slp", "MAge_1_slp")), yy = 0), 
             aes(yintercept = yy), linetype = 2) + 
...
F_FMSY <- lapply(1:length(MSE), function(x, MPs) MSE[[x]]@F_FMSY[, match(MPs, MSE[[x]]@MPs), , drop = FALSE] %>% 
                   ...
                   mutate(OM = paste0("NR", x)), MPs = "M02")
F_FMSY <- do.call(rbind, F_FMSY)

ggplot(filter(indicators_scatter, MP == "M02") %>% left_join(F_FMSY) %>% filter(!is.na(F.FMSY)), 
