...
Cbias <- c(2.25, 1.25, 1)
create_target_MP <- function(mod, pstar = 0.5, Cbias = 1, delta = c(0.8, 1.2)) {
  ...
  MP <- function(x, Data, reps, AddInd = 1) {
    ...
    if(is.character(Itarget)) {
      ...
      Cat <- Data@Cat[x, length(Data@Year)]/Cbias
...
for(i in 1:3) {
  ...
  tMP_20cap <- create_target_MP(mod, Cbias = Cbias[i])
  tMP_nocap <- create_target_MP(mod, Cbias = Cbias[i], delta = c(0, 1000))
  MSE <- runMSE(SRA_OM@OM, MPs = c("tMP_20cap", "tMP_nocap"))
